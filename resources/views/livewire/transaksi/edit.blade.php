<div>
    <x-header title="Edit Transaksi" separator progress-indicator>
        <x-slot:actions>
            <x-button label="Print Struk" wire:click="printReceipt" icon="o-printer" class="btn-ghost" />
        ...
        <x-form wire:submit="save">
            <div class="space-y-5">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <x-input label="ID Transaksi" wire:model="formData.id_transaksi" readonly
                        hint="ID tidak dapat diubah" icon="o-hashtag" />
                    ...
                    <x-select label="Pilih Pelanggan" wire:model.live="formData.id_pelanggan" :options="$pelangganList"
                        option-value="id" option-label="name" icon="o-user" placeholder="Pilih pelanggan..." required />

                    <x-select label="Pilih Layanan" wire:model.live="formData.id_layanan" :options="$layananList"
                        option-value="id" option-label="name" icon="o-sparkles" placeholder="Pilih layanan..." required />
                </div>

                <!-- Jenis Pakaian Component -->
                <livewire:components.jenis-pakaian-input :value="$formData['jenis_pakaian']" :key="'edit-jenis-pakaian-' . $idTransaksi" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-datetime label="Tanggal Selesai" type="datetime-local" wire:model="formData.tanggal_selesai"
