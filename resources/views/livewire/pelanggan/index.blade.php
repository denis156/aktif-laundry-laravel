<div>
    ...
    <x-header title="Daftar Pelanggan" separator progress-indicator>
        ...
        <x-slot:actions>
            @if($search || $statusFilter || $minTransaksi > 0 || $maxTransaksi < 999999)
                <x-badge value="Filter Aktif" class="badge-warning" />
            @endif
            <x-button label="Tambah Pelanggan" link="/pelanggan/create" responsive icon="o-plus" class="btn-success" />
    ...
    <x-card class="shadow-sm">
        <x-table :headers="$headers" :rows="$pelanggan" :sort-by="$sortBy" striped with-pagination per-page="perPage" :per-page-values="[5, 10, 25, 50]">
            ...
            <a href="https://wa.me/62{{ ltrim($item['no_hp'], '0') }}" target="_blank" class="link link-primary">
                {{ $item['no_hp'] }}
            </a>
            @endscope
            ...
            <div class="flex items-center justify-end gap-2">
                <x-button
                    label="Edit"
                    icon="o-pencil"
                    link="/pelanggan/edit/{{ $item['id_pelanggan'] }}"
                    class="btn-sm btn-ghost hover:btn-info"
                />
                <x-button
                    label="Hapus"
                    icon="o-trash"
                    wire:click="confirmDelete('{{ $item['id_pelanggan'] }}', '{{ $item['nama'] }}')"
    ...
    <x-drawer wire:model="drawer" title="Filter Pelanggan" subtitle="Saring data sesuai kebutuhan" right separator with-close-button class="lg:w-1/3">
        <div class="space-y-5">
            ...
            <x-input
                label="Pencarian"
                placeholder="Cari nama, HP, ID, atau email..."
            ...
            <!-- Transaksi Range Filter -->
            <div class="space-y-3">
                <label class="block text-sm font-semibold">Range Total Transaksi</label>
                <div class="grid grid-cols-2 gap-3">
                    <x-input
                        label="Minimal"
                        type="number"
                        wire:model.live.debounce="minTransaksi"
                        placeholder="0"
                        suffix="x"
                    />
                    <x-input
                        label="Maksimal"
                        type="number"
                        wire:model.live.debounce="maxTransaksi"
                        placeholder="999999"
                        suffix="x"
                    />
                </div>
